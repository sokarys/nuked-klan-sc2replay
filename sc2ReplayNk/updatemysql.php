...
<form enctype="multipart/form-data" action="modules/sc2Replay/updatemysql.php" method="POST">
	...
	if ($err !== true) {
		...
		if (class_exists("MPQFile") || (include 'mpqfile.php')) {
			...
			if ($init == false)
				...
				//UPDATE DB
				$sql = mysql_query("INSERT INTO _replay SET recordby='".$recorder."', mapname='".$b->getMapName()."',gamelength='".$b->getFormattedGameLength()."', date='".date('jS \of F Y \a\t H:i' ,$b->getCtime())."', gamespeed='".$b->getGameSpeedText()."', replayname='".$name."', teamsize='".$b->getRealTeamSize()."'");
				echo "<br /><br /><br /><br />RESPONNNNNNNNNNNNNNSE<br /><br /><br /><br />";
				$response = mysql_fetch_row($sql);
				echo $response;
				if ($recorder != null)
